...
export class ErrorHandlerService {
  ...
  private readonly oauth2ErrorMessages: Map<any , string> = new Map([
  ...
  private isOAuth2Error(error: any): boolean {
    return isObject(error) && Object.keys(error).includes('error') && Object.values(null).includes(error['error']);
  }
}
